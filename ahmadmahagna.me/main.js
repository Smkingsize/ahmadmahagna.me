...
renderer.setClearColor(0x281C3C); // Space color background
document.body.appendChild(renderer.domElement);

// Load the texture
const textureLoader = new THREE.TextureLoader();
const backgroundTexture = textureLoader.load('./assets/space.jpg'); // Update the path to your image

// Create a large plane geometry
const planeGeometry = new THREE.PlaneGeometry(100, 70); // Large plane to cover the background
const planeMaterial = new THREE.MeshBasicMaterial({ map: backgroundTexture });
const plane = new THREE.Mesh(planeGeometry, planeMaterial);

// Position the plane behind everything in the scene
plane.position.z = -100; // Make sure it's behind all other objects
scene.add(plane);
...
const ringGeometry = new THREE.RingGeometry(1, 1.5, 530); // Creates a ring with inner and outer radii
...
window.addEventListener('scroll', () => {
    ...
    scrollAmount = scrollY * 0.01; // Scroll factor, tune this for more/less sensitivity
    camera.position.z = 5 + scrollAmount; // Base Z position is 5, modify with scroll
});
